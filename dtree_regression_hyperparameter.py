...
X_train,y_train = generate_data(n_samples=n_train,noise=noise)
X_test,y_test = generate_data(n_samples=n_test,noise=noise)

plt.style.use("fivethirtyeight")

st.sidebar.markdown("# Decision Tree Regressor",unsafe_allow_html=True)

criterion = st.sidebar.selectbox("Criterion",("mse",'friedman_mse','mae'))
splitter = st.sidebar.selectbox("Splitter",("best",'random'))
max_depth = int(st.sidebar.number_input("Max Depth",min_value=5,step=5))
min_samples_split = st.sidebar.slider("Min Samples Split",1,X_train.shape[0],2,key=1234)
min_samples_leaf = st.sidebar.slider("Min Samples Leaf",1,X_train.shape[0],1,key=1345)
...
ax.scatter(X_train,y_train,color='yellow',edgecolors='black')
orig = st.pyplot(fig)
...
if st.sidebar.button("Run Algorithm"):
    ...
    st.subheader(f"R2 Score:{round(reg_r2,2)}")
    ax.scatter(X_train,y_train,color='yellow',edgecolors='black')
